...
<?php if ( comments_open() || have_comments() ) : ?>
	...
	<div id="comments" class="comments-area">
		...
		<?php 
		    $loginstr =  '<p class="must-log-in">' . sprintf(
		    ...
		    if (is_weixin()) 
			{			
		        //$link =  apply_filters( 'the_permalink', get_permalink( get_the_ID()) ) ;
				$link  = '<a onclick="login_button_click(\'wechat\','.'\''. apply_filters( 'the_permalink', get_permalink( get_the_ID()) )  .'\')">登录</a>';
				$loginstr = '<p class="must-log-in">' . sprintf(
		                              /* translators: %s: login URL */
		                              __( '你必须先 %s 才能发表评论' ),
		                             $link
		   ...
		   comment_form(array(
                    ...
                    'logged_in_as' => '',
                    'comment_notes_after' => '',
                    'comment_field' =>  '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="5" aria-required="true" placeholder="'.__('Your Comment...', 'boss').'"></textarea></p>',
                    'label_submit' => __('Comment', 'boss'),
					'must_log_in' =>$loginstr 					
                )); 
            ?>
