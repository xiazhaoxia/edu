	...
	{   
	   ...
	   function eventCheck(e) 
	   {
		  ...
		  {  
			 ...
			 {
				$('#mobile-bottom-bar').css("display","none");
			 }
			 else
			 {
				clearInterval(timer);
				 
				$('#mobile-bottom-bar').css("display","block");
			 }
		  }
	   }
	
	   /*,.sap-editable-title,.sap-editable-area*/
	   $('input[type=text],input[type=password],textarea,.sap-editable-title,.sap-editable-area').click(eventCheck).blur(eventCheck);
	   
	    
	    if($("#mobile-header li").length == 0)
	    {
		   $("#mobile-header").css("display","none");
		   $("#inner-wrap").css("margin-top","0");
	    }
		else
		{
		   $("#mobile-header").css("display","block");
		   $("#inner-wrap").css("margin-top","45px");
		}
	}
);
