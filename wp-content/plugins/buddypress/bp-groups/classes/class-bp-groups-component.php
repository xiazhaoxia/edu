	...
	public function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		if ( bp_is_user() ) {
			...
			$nav_name = sprintf(
				...
				_x( '群 %s', 'Group screen nav with counter', 'buddypress' ),
				sprintf(
					'<span class="%s">%s</span>',
					esc_attr( $class ),
			...
			$nav_name = _x( '群', 'Group screen nav without counter', 'buddypress' );
		}

		$slug = bp_get_groups_slug();
		$homegroupurl = trailingslashit(home_url($slug));
		...
		if ( bp_is_groups_component() && bp_is_single_item() ) {
			...
			$sub_nav[] = array(
				'name'            => _x( '课程', 'My Group screen nav', 'buddypress' ),
				'slug'            => 'course',
				'parent_url'      => $group_link,
				'parent_slug'     => $this->current_group->slug,
				'screen_function' => 'groups_screen_group_course',
				'position'        => 40,
				'user_has_access' => $this->current_group->user_has_access,
				'item_css_id'     => 'course'
				);
			...
			{		
				    $sub_nav[] = array(
					'name'            => _x( '写课程', 'My Group screen nav', 'buddypress' ),
					'slug'            => 'createpost',
					'parent_url'      => $group_link,
					'parent_slug'     => $this->current_group->slug,
					'screen_function' => 'groups_screen_create_course',
					'position'        => 50,
					'user_has_access' => $this->current_group->user_has_access,
					'item_css_id'     => 'createcourse'
					);
			}		
