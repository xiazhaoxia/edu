	...
	public function prepare_user_ids_query() {
		...
		switch ( $type ) {
			...
			case 'popular' :
				...
				$this->uid_name = 'user_id';
				$this->uid_table = $bp->groups->table_name_members;
                $sql['select']  = "SELECT DISTINCT u.{$this->uid_name} as id FROM {$this->uid_table} u inner join (SELECT post_author,sum(CONVERT(wp_postmeta.meta_value, SIGNED)) viewtotal FROM {$wpdb->posts} left join {$wpdb->postmeta} on ({$wpdb->posts}.ID = {$wpdb->postmeta}.post_id and {$wpdb->postmeta}.meta_key='views') WHERE post_type = 'course' GROUP BY post_author) as c on (u.user_id = c.post_author)" ;
				//$sql['where'][] = $wpdb->prepare( "u.meta_key = %s", bp_get_user_meta_key( 'total_friend_count' ) );
				$sql['orderby'] = "ORDER BY c.viewtotal ";
				$sql['order']   = "DESC";
				break;
