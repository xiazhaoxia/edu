...
function open_init() {
	...
	$GLOBALS['open_arr'] = array(
		...
		'wechatqr'=>__('WeChatQR','open-social'),
		'wechatle'=>__('WeChatLE','open-social'),
	);
...
function open_isbind($open_type,$open_id) {
	...
	if(!$bid){//single era
		...
		if($bid){
			update_user_meta($bid, 'open_type_'.$open_type, $open_id);
...
class BASE_CLASS
{
	...
	function save_extra_data($wpuid,$newuser)
	{
	...
	function validate()
	{
		...
		if (!isset($_SESSION['access_token'])
		   ||strlen($_SESSION['access_token'])<6
		   )
			return false ;
			
		if(!isset($_SESSION['open_id'])
			||strlen($_SESSION['open_id'])<6) 
			return false;
	...
	{	
		...
		$newuser   = $this->open_new_user();	
        $open_type = $this->getOpenType();
		$uid       = $this->getUId();
		
	    if (is_user_logged_in()) { //bind
			$wpuid = get_current_user_id();

			if ( $dbuid = open_isbind($open_type,$uid ) ) {
				if($dbuid != $wpuid)
				{
		...
		{
			$wpuid = open_isbind($open_type,$uid);
			if (!$wpuid) {
				$wpuid = username_exists(strtoupper($open_type).$uid);
				if(!$wpuid)
				{
					...
					$userdata = array(
						...
						'user_login' => strtoupper($open_type).$uid
					);
					...
					$bpsignup = array(
						...
						'activation_key' => $userdata['user_pass'] 
						 
					);
					$usermeta =  array_diff_key($userdata,$bpsignup);
					$usermeta["password"] = $usermeta["user_pass"];
					unset($usermeta["user_pass"]);
					$bpsignup['meta'] = $usermeta;
					add_signup($bpsignup);
					
					$this->save_extra_data($wpuid,$newuser);
				}
			} 
		} 
		
		//update open type, open_img, access_token
		if($wpuid){
			...
			update_user_meta($wpuid, 'open_type_'.$open_type, $uid);
...
class QQ_CLASS extends BASE_CLASS  {
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class SINA_CLASS extends BASE_CLASS {
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class BAIDU_CLASS extends BASE_CLASS {
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class GOOGLE_CLASS extends BASE_CLASS{
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class LIVE_CLASS extends BASE_CLASS {
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class DOUBAN_CLASS extends BASE_CLASS {
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class RENREN_CLASS extends BASE_CLASS{
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class KAIXIN_CLASS extends BASE_CLASS{
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class XIAOMI_CLASS extends BASE_CLASS {
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class CSDN_CLASS extends BASE_CLASS {
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class OSCHINA_CLASS extends BASE_CLASS{
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class FACEBOOK_CLASS extends BASE_CLASS{
	function open_login() {
		$params=array(
			'response_type'=>'code',
...
class TWITTER_CLASS extends BASE_CLASS{
	function open_login() {
		$str = '';
		$params=array(
...
class GITHUB_CLASS extends BASE_CLASS{
	function open_login() {
		$params=array(
...
class WECHATQR_CLASS extends BASE_CLASS{
	protected $newuser;
	...
	function open_login() {
		$params=array(
			'appid'=>osop('WECHATQR_AKEY'),
			'redirect_uri'=>home_url('/').'?connect=wechatqr&action=callback',
			'response_type'=>'code',
			'scope'=>'snsapi_login',
			'state'=>md5(uniqid(rand(), true))
		);
	...
	function open_new_user(){
		...
		$this->newuser = array(
			...
			'user_url'     => '',
			'user_email'   => $user['unionid'].'@fake.com',//strtoupper(OPEN_TYPE).$_SESSION['open_id'].'@fake.com',
    ...
    function getUId(){
        return  $this->newuser['unionid'] ;
    }
		

	function save_extra_data($wpuid,$newuser)
	{
		update_user_meta($wpuid, 'sex', $newuser['sex']);
		update_user_meta($wpuid, 'province', $newuser['province']);
		update_user_meta($wpuid, 'city', $newuser['city']);
		update_user_meta($wpuid, 'country', $newuser['country']);	
	}	
} 

class WECHAT_CLASS  extends BASE_CLASS{
	protected $newuser;
	function open_login() {
		$params=array(
	...
	function open_new_user(){
		...
		$this->newuser=  array(
			...
			'country'      => $user['country'],
			'user_url'     => '',
			'user_email'   => $user['openid'].'@fake.com',//strtoupper(OPEN_TYPE).$_SESSION['open_id'].'@fake.com',
			'unionid'      => $user['openid']
		);
		return $this->newuser;
	} 
...
class WECHATLE_CLASS  extends BASE_CLASS{
	protected $newuser;
	...
	function open_login() {
		$params=array(
			'redirect_url'=>home_url('/').'?connect=wechatle&action=callback',
			'state'=>md5(uniqid(rand(), true))
		);
         
		$requrl = 'Location:http://passport.lelianyanglao.com/api/login-page?'.http_build_query($params);
		header( $requrl);	
		//header('Location:http://passport.lelianyanglao.com/api/login-page?redirect_url='.http_build_query($params) );
		exit();
	} 
	...
	function open_new_user(){
		$url = 'http://passport.lelianyanglao.com/api/check-token';
		$param = [
			'token' =>$_GET['token'],
		];

		$res = file_get_contents('compress.zlib://' . $url. '?' . http_build_query($param));
		...
		$this->newuser=   array(
			...
			'country'      => $user['country'],
			'user_url'     => '',
			'user_email'   => $user['uid'].'@fake.com',//strtoupper(OPEN_TYPE).$_SESSION['open_id'].'@fake.com',
			'unionid'      => $user['uid'],
			'mobile'       => $user['mobile'],
	...
	function save_extra_data($wpuid,$newuser)
	{
		...
		update_user_meta($wpuid, 'city', $newuser['city']);
		update_user_meta($wpuid, 'country', $newuser['country']);
		update_user_meta($wpuid, 'mobile', $newuser['moible']);
		update_user_meta($wpuid, 'subscribe', $newuser['subscribe']);		
		update_user_meta($wpuid, 'subscribe_time', $newuser['subscribe_time']);	
	}	
} 
...
function open_connect_http($url, $postfields='', $method='GET', $headers=array()){
	$ci = curl_init();
	if(osop('proxy_server') && preg_match('/facebook.com|twitter.com|google.com/', $url))
		curl_setopt($ci, CURLOPT_PROXY, osop('proxy_server'));
	...
	if(count($json_r)==0){
		...
		if(count($json_r)==1 && current($json_r)==='') 
			return $response;
	}
	return $json_r;
}
...
function open_options_page() {
	...
	<div class="wrap">
		...
		<tr valign="top">
		<th scope="row"><?php _e('Login Buttons','open-social')?><br/>
			<a href="<?php echo admin_url('widgets.php');?>"><?php _e('Widgets');?></a></th>
		<td><fieldset>
			...
			<label for="osop[show_login_form1]"><input name="osop[show_login_form]" id="osop[show_login_form1]" type="radio" value="1" <?php checked(osop('show_login_form'),1);?> /> <?php _e('Before comment form','open-social')?></label> 
			<label for="osop[show_login_form2]"><input name="osop[show_login_form]" id="osop[show_login_form2]" type="radio" value="2" <?php checked(osop('show_login_form'),2);?> /> <?php _e('After comment form','open-social')?></label>  
			<label for="osop[show_login_form0]"><input name="osop[show_login_form]" id="osop[show_login_form0]" type="radio" value="0" <?php checked(osop('show_login_form'),0);?> /> <?php _e('None');?></label> <br/>
			<pre>Shortcode: <code>[os_login]</code> <code>[os_login show="qq,sina"]</code> PHP: <code>&lt;?php echo open_social_login_html();?&gt;</code></pre>
		</fieldset>
		</td>
		</tr>
		<tr valign="top">
		<th scope="row"><?php _e('Share Buttons','open-social')?><br/>
			<a href="<?php echo admin_url('widgets.php');?>"><?php _e('Widgets');?></a></th>
		<td><fieldset>
			...
			<a href="http://shang.qq.com/widget/set.php" target="_blank"><?php _e('QQChat Number','open-social')?></a></p>
			<?php
		...
		<td><fieldset>
			<label for="osop[extend_guest_comment]"><input name="osop[extend_guest_comment]" id="osop[extend_guest_comment]" class="regular-text" placeholder="/:\/\//" value="<?php echo osop('extend_guest_comment')?>" /> <?php _e('Regexp Anti-SPAM when guest can comment','open-social')?></label><br/>
			...
			<label for="osop[extend_lang_switcher]"><input name="osop[extend_lang_switcher]" id="osop[extend_lang_switcher]" type="checkbox" value="1" <?php checked(osop('extend_lang_switcher'),1);?> /> <?php _e('Display Language Switcher in profile page','open-social')?></label>
			<a href="<?php echo admin_url('profile.php');?>#admin_bar_front">#<?php _e('Profile');?></a><br/>
			<label for="osop[extend_user_role]"><input name="osop[extend_user_role]" id="osop[extend_user_role]" type="checkbox" value="1" <?php checked(osop('extend_user_role'),1);?> /> <?php _e('User Subscriber role for new user or default role if uncheck','open-social')?>
		...
		<tr valign="top">
			<th scope="row"><?php _e('Proxy','open-social')?></th>
		<td><fieldset>
			...
			<a href="https://www.xiaomac.com/2014081490.html" target="_blank"><?php _e('Proxy for somesite in somewhere','open-social')?></a><br/>
	...
	<div class="wrap">
		...
		<?php
			$open_arr_link = array(
				...
				'wechatqr'=> array('https://open.weixin.qq.com/cgi-bin/index','https://open.weixin.qq.com/cgi-bin/index'),
				'wechatle'=> array('https://open.weixin.qq.com/cgi-bin/index','https://open.weixin.qq.com/cgi-bin/index')
			);
...
function open_social_login_html($atts=array()) {
	...
	if( osop('show_inner_login',1) && stripos(wp_login_url(),basename( $_SERVER['SCRIPT_NAME'] ))===false ) {
...
function open_login_button_show($icon_type,$icon_title,$icon_link){
    if($icon_type == 'wechatqr' || $icon_type == 'wechatle')
	   $k = 'wechat';
    else
	   $k = $icon_type;
	return "<div class=\"login_button login_icon_$k\" id = $icon_type onclick=\"login_button_click('$icon_type','$icon_link')\" title=\"$icon_title\"></div>";
}
