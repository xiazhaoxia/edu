...
jq( function () {
    jq( '.sap-editable-area' ).mediumInsert( {
         ...
         addons: { // (object) Addons configuration
                images: { // (object) Image addon configuration
					...
					//actions:null,
                    deleteScript: ajaxurl+"?action=buddyboss_sap_delete_photo&sap-editor-nonce="+$( "input[name='sap_editor_nonce']" ).val(), // (string) A relative path to a delete script
                    deleteMethod: 'POST',
                    ...
                    captionPlaceholder: '点击这里写正文', // (string) Caption placeholder
                    autoGrid: 100, // (integer) Min number of images that automatically form a grid
                    fileUploadOptions: { // (object) File upload configuration. See https://github.com/blueimp/jQuery-File-Upload/wiki/Options
                        ...
                        formData: [
                        {name:'sap-editor-nonce',value: $( "input[name='sap_editor_nonce']" ).val()}
                        ],
	...
	jq('#sap-cat option:contains("原创")').attr("selected",true);
    //jq('#sap-cat').val('1');
    jq('#sap-cat').change();	
} );
...
jq( '.sap-story-draft' ).on( 'click', function ( e ) {
    ...
    jQuery(".sap-story-publish").text('提交');
...
jq( '.sap-story-delete' ).on( 'click', function ( e ) {
	...
	var r = confirm( "一旦删除就无法恢复. 你确定要删除?" );
    if ( r == true ) {
...
function sap_draft_screen_auto_save() {
    ...
    leavetype = function () {
        if($(this).find('.medium-insert-images').length > 0 
		   || $(this).find('medium-insert-embeds').length > 0 
		   || $(this).find('p').html() !== '<br>') 
		{
...
function sap_post_add_screen( visibility, draft ) {
    ...
    var is_index_page = jq('#sap_is_index_page').val();
 	...
 	var post_classroom_id = jq('#classroomid').val();

    if ( post_visibility !== undefined && post_visibility.length > 0 ) {
	...
	if( data.post_content.value.indexOf("base64") != -1)
	{
		return;
	}
    //data = jq.param( data );
        ...
        success: function ( response ) {
			try 
			{
				...
				if ( 'pending' == visibility && response[0] != 'Empty' ) {
					...
					jq( '.sap-story-delete' ).show();
					window.location.href = response[1];jq( '#pending-url' ).val();				
				}
			} catch(err) {
...
function sap_check_empty_title() {
    ...
    if ( 'true' != edit_status && jq(check_title).text().length == 0 ) {
        return 'empty';
    }
...
function sap_empty_editor() {
	...
	//jq('#sap-cat').find("option[text='原创频道']").attr("selected",true);
	jq('#sap-cat option:contains("原创")').attr("selected",true);
    //jq('#sap-cat').val('1');
    jq('#sap-cat').change();
...
( function ( context, window, $ ) {
    ...
    context.init = function () {
        ...
        $( '.toggle-sap-widgets' ).on( 'click', function ( e ) {
            e.preventDefault();
            $( this ).toggleClass( 'active' );
            $( this ).find( 'i' ).toggleClass( 'fa-pencil-square-o fa-times' );
            $( '.sap-widget-container' ).slideToggle(250);
			
				 
			var dv = _l.$open_uploder_button.parent().find('.moxie-shim');

			//dv.css("width","100%");
			dv.css("width",_l.$open_uploder_button.css("width")); 
			 
			//dv.css("height","100%");
			dv.css("height",_l.$open_uploder_button.css("height")); 			
        } );

        //for adding new tags
        $( '#input-tags-select' ).selectize( {
    ...
    context.get_elements = function () {
        _l.$form = $( '.sap-container-wrapper' );//CHANGE THIS

        if ( _l.$form.length === 0 ) {
            return false;
        }

        _l.$open_uploder_button = $( '#featured-img-placeholder' );
    ...
    context.start_uploader = function () {
        ...
        uploader = new plupload.Uploader( {
            ...
            multipart_params: {
                ...
                'sap-editor-nonce': $( "input[name='sap_editor_nonce']" ).val()//CHANGE THIS
            },
            filters:  
			{
                mime_types:[
...
jQuery( document ).ready( function ( $ ) {
	...
	// jq('.sap-editable-title').on('focus',function(){
       // $("#mobile-bottom-bar").css("display","none");
      // }
    // )
	
	// jq('.sap-editable-title').on('blur',function(){
       // $("#mobile-bottom-bar").css("display","block");
      // }
    // )	
	
	// jq('.sap-editable-area').on('focus',function(){
       // $("#mobile-bottom-bar").css("display","none");
      // }
    // )

	// jq('.sap-editable-area').on('blur',function(){
       // $("#mobile-bottom-bar").css("display","block");
      // }
    // )	
...
$( document ).on( 'ready', function () {
	...
	jq( document ).on( 'scroll', function () {
		...
		var load_more_btn = jq( ".sap-load-more-posts" );
	...
	function load_more_posts() {
		...
		var load_more_btn = jq( "div a.sap-load-more-posts" );
			 
		
		var   
            sort  = load_more_btn.attr( 'data-sort' ),
            max   = load_more_btn.attr( 'data-max' ),
        ...
        $.ajax( {
            ...
            success: function ( html ) {
                ...
                // Append Post Markup
                load_more_btn.parent().before( html );
