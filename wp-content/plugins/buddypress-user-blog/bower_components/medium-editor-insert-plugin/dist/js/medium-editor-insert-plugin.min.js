...
;(
 function ($, window, document, undefined)
 {
    ...
    Core.prototype.toggleButtons = function (e) {
        ...
        if ($el.hasClass('medium-editor-placeholder') === false 
      ...
      && $current.closest('.medium-insert-buttons').length === 0) {
        ...
        console.log("add class medium-insert-active");
...
; (function ($, window, document, Util, undefined) 
{
   ...
   //add Images except android and weixin(android)
    Images.prototype.addDefaultImage=function(){
        var that = this,
    ...
    //xiazhaoxia
    //2017.4.13
    //support mutiple image update on android by wx 
    */
    Images.prototype.addWxImage=function(){
        var _option={
            wxUploadComplete:function(){
                //重写uploadcompleted
        ...
        $.extend(true, {}, this.options,_option);
        var that=this;
        wx.ready(function () {
            // 在这里调甿API
            ...
            wx.chooseImage({
                count: 9, // 默认9
                sizeType: ['original', 'compressed'], // 可以指定是原图还是压缩图，默认二者都朿
                sourceType: ['album', 'camera'], // 可以指定来源是相册还是相机，默认二者都朿
                success: function (res) {
                    //
          
                    //加入预览功能
                    $.proxy(that, 'wxLocalPreview', res.localIds)();   
                    //上传图片到微信服务器
                    $.when($.proxy(that, 'uploadWxImage', res.localIds)()).done(function(ids,imagesData){
            
            //alert("ids----:----"+JSON.stringify(ids));
            //alert("imageData-----:-------"+JSON.stringify(imagesData));
            $.proxy(that, 'uploadWeImage', ids,imagesData)();
          });                     
                }
            });                 
        });
    ...
    //2017.4.13
    //上传图片到微信服务器
    Images.prototype.uploadWxImage=function(ids){
    var dtd=$.Deferred();
    var that=this;
    ...
    $.each(ids,function(index,id){
      ...
      that.uploadWxImageEach(id).done(function(serverId){
        _imagesId.push(serverId);
        _images.push({
          localId:id,
          serverId:serverId
        });
        
        if(_images.length==ids.length){       
          dtd.resolve(_imagesId,_images);
        }
      });
  ...
  Images.prototype.uploadWxImageEach=function(localId){
  
    
    var dtd=$.Deferred();   
    wx.uploadImage({
      localId:localId, // 需要上传的图片的本地ID，由chooseImage接口获得
      isShowProgressTips: 1, // 默认丿，显示进度提礿
      success: function (res) {   
        dtd.resolve(res.serverId);
      }
    }); 
  ...
  //
  Images.prototype.uploadWeImage=function(serverIds,imageData){
    var that=this;
  
    var data={};
    data.wxImageIds=serverIds.join(",");
    data.action="sp_get_wximg"
    $.ajax({
      type:"post",
      dataType:"json",
      ...
      success:function(res){                
        for(var j=0;j<res.length;j++){
          imageData[j].imgUrl=res[j];                                       
        }  
        
        $.proxy(that, 'showWxImageByserver', imageData)();
      },
      fail:function(res){
        console.log(res);
      }
    ...
    //微信图片预览，只需要提便
    Images.prototype.wxLocalPreview=function(localIds){
        for(var i=0;i<localIds.length;i++){
          ...
          if ($place.is('p')) {
            ...
            $place.after('<div class="medium-insert-active"> </div>');
            $place = this.$el.find('.medium-insert-active');
              if ($place.next().is('p')) {
                  this.core.moveCaret($place.next());
              } else {
                   //$place.after('<p><br></p>'); // add empty paragraph so we can move the caret to the next line.
                  console.log("add p by upload");
                   //this.core.moveCaret($place.next());
              }
          }
          ...
          $.proxy(this, 'showWxImage',localIds[i])();
        }
    }
  Images.prototype.showWxImageByserver=function(wxImages){
    ...
    $.each(wxImages,function(index,img){
      domImage=that.getDOMImage();
      domImage.localId=img.localId;
      domImage.imgUrl=img.imgUrl;


      domImage.onload = function () {
        
        //alert("current image----"+JSON.stringify(this));        
        $("[src='"+this.localId+"']").attr("src",this.imgUrl);
        
        
        
        if (that.options.wxUploadComplete) {
            that.options.wxUploadComplete();
          }
        that.core.triggerInput();
        dtd.resolve();
      ...
      domImage.onerror=function(e){
        alert("image load error");
        dtd.reject();
      }
      domImage.src = img.imgUrl;
      //domImage.src="https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1492349508639&di=0a2957e85bb4649073b8f029276686d9&imgtype=0&src=http%3A%2F%2Fpic6.huitu.com%2Fres%2F20130116%2F84481_20130116142820494200_1.jpg";
    });
    return dtd.promise();
  }
    ...
    //微信图片预览
    Images.prototype.showWxImage=function(img){
        var $place = this.$el.find('.medium-insert-active'),
            that;
    ...
    var $feature=$(this.templates['src/js/templates/images-image.hbs']({
      img: img
    })).appendTo($place);   
    ...
    if (this.options.autoGrid && $place.find('figure').length >= this.options.autoGrid) {
      ...
      if (this.options.styles.grid.added) {
        ...
        if (this.options.wxUploadComplete) {
            this.options.wxUploadComplete();
        }
    ...
    Images.prototype.add = function () {
        ...
        var _version=window.weilele.browser.versions;

        if(_version.weixin){
            console.log("wexin");
      $.proxy(this,"addWxImage")();

        }else if(_version.android){
            console.log("android");

        }else{
            $.proxy(this,"addDefaultImage")();
        }
    ...
    else
    {
        ...
        if (data.autoUpload || (data.autoUpload !== false && $(e.target).fileupload('option', 'autoUpload'))) {
            data.process(
        function () {
                    return $(e.target).fileupload('process', data);
                }
        ).done(function () {
    ...
    Images.prototype.showImage = function (img, data) {
        ...
        if (this.options.preview && data.context) {
            ...
            domImage.onload = function () {
        var imgelements = data.context.find('img')
        ...
        {
            var imgelements = data.context.find('br') 
            if(imgelements.length != 0)
          {
            imgelements.remove();  
          }
